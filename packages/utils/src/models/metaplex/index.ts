...
export class AuctionManager {
  ...
  constructor(args: {
    ...
    this.safetyDepositBoxesExpected =
      args.instance.info.key === MetaplexKey.AuctionManagerV2
        ? new BN(args.vault.info.tokenTypeCount)
        : new BN(
            (
              args.instance.info as AuctionManagerV1
            ).state.winningConfigItemsValidated
          );
    this.store = args.instance.info.store;
    this.authority = args.instance.info.authority;
    this.vault = args.instance.info.vault;
    this.acceptPayment = args.instance.info.acceptPayment;
    this.auction = args.instance.info.auction;
    this.status = args.instance.info.state.status;
    ...
    this.participationConfig =
      args.instance.info.key == MetaplexKey.AuctionManagerV2
        ...
        : (args.instance.info as AuctionManagerV1).settings
  ...
  getItemsFromSafetyDepositBoxes(
    ...
    if (
      this.instance.info &&
      this.instance.info.key == MetaplexKey.AuctionManagerV1
    ) {
      try {
        return (
