...
export const MAX_AUCTION_DATA_EXTENDED_SIZE_ORIGINAL =
  8 + 9 + 2 + 9 + 33 + 9 + 9 + 9 + 158;
...
export class AuctionDataExtended {
  ...
  name: number[] | null;
  decreaseRate: BN | null;
  decreaseInterval: BN | null;
  auctionStartTime: BN | null;
  ...
  constructor(args: {
    ...
    this.name = args.name;
    this.decreaseRate = args.decreaseRate;
    this.decreaseInterval = args.decreaseInterval;
    this.auctionStartTime = args.auctionStartTime;
  }
}
...
export class AuctionData {
  ...
  public ended() {
    // If there is no endedAuctionAt, this is an instantSale Auction
    if (
      (this.bidState.type === 2 ||
        this.bidState.type === 1 ||
        !this.endAuctionAt) &&
      this.lastBid
    ) {
      return true;
  ...
  [
    AuctionDataExtended,
    {
      ...
      fields: [
        ...
        ['instantSalePrice', { kind: 'option', type: 'u64' }],
        ['name', { kind: 'option', type: [32] }],
        ['decreaseRate', { kind: 'option', type: 'u64' }],
        ['decreaseInterval', { kind: 'option', type: 'u64' }],
        ['auctionStartTime', { kind: 'option', type: 'u64' }],
      ],
    },
  ],
  [
