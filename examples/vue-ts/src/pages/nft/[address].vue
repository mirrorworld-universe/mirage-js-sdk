<template>
  <chakra.main :p="[10, 10, 50]" color="white" font-weight="regular" min-h="100vh">
    <c-container max-w="1200">
      <c-button left-icon="arrow-left" :mb="[10, 10, 10]" as="router-link" to="/">
        Home
      </c-button>
      <template v-if="nft">
        <c-grid :template-columns="['1fr', '1fr', '1fr 1.5fr']" gap="8"  align-items="flex-start">
          <c-stack spacing="8">
            <c-aspect-ratio :ratio="1" :min-w="['full', 'full', '250px']" rounded="lg"
              overflow="hidden"
          ...
          <c-stack spacing="8">
            ...
            <c-stack>
              <template v-if="userIsOwner">
                <c-h-stack>
                  <template v-if="!tokenIsOnSale">
                    <c-button size="lg" color-scheme="purple" left-icon="dollar-sign" @click="() => {
                  ...
                  <c-button size="lg" left-icon="gift" variant="outline" @click="() => {
              ...
              <template v-else-if="userIsOwner && tokenIsOnSale">
                <c-button size="lg" left-icon="gift" variant="outline" @click="cancelListing">
                  Gift
                </c-button>
              </template>
              <template v-else>
                <c-button :is-disabled="!tokenIsOnSale" @click="buyNft">
                  Buy Now
                </c-button>
              </template>
            </c-stack>
                ...
                <c-stack spacing="2">
                  <c-h-stack v-for="(receipt) in transactions" :key="`receipt-${i}`" w="full" justify-content="space-between">
                    <c-h-stack>
                      ...
                      <c-stack>
                        <c-text>
                          <c-link as="router-link" text-decor="underline" color="purple.300" :to="`/profile/${nftOwner}`">{{ receipt.bookkeeper.toBase58() === wallet.publicKey?.toBase58() ? "You" : truncateMiddle(receipt.bookkeeper.toBase58(), 4, 4, '. . .') }}</c-link>
                        </c-text>
                    ...
                    <c-text>{{ receipt.receipt_type === 'listing_receipt' ? 'Listed for' :  receipt.receipt_type === 'bid_receipt' ? 'Offered' : receipt.receipt_type === 'purchase_receipt' ? "Purchased for": "Cancel listing" }} </c-text>
