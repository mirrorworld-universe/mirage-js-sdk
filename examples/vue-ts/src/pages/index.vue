    ...
    <c-container max-w="1200">
      <c-v-stack spacing="8" align-items="flex-start">
        <c-h-stack align-items="center" justify-content="space-between" w="full">
          ...
          <c-h-stack spacing="4">
            <c-icon-button as="a" rounded="full" size="lg" target="_blank" href="https://github.com/mirrorworld-universe/mirage-js-sdk#readme" icon="book" aria-label="documentation" />
            <c-icon-button as="a" rounded="full" size="lg" target="_blank" href="https://github.com/mirrorworld-universe/mirage-js-sdk" icon="github" aria-label="github" />
            <c-icon-button as="a" rounded="full" size="lg" target="_blank" href="https://github.com/mirrorworld-universe/mirage-js-sdk/tree/main/examples/dapp" icon="code" aria-label="source code" />
          </c-h-stack>
        </c-h-stack>
        <c-stack px="6" py="4" backdrop-filter="blur(5px)" border-width="1px" border-style="solid" border-color="whiteAlpha.500">
          <c-text>
            MirrorWorld Marketplace JS SDK. Used to manage the following:
          </c-text>
          <c-list spacing="2">
            <c-list-item>
        ...
        <template v-if="!pk">
          <c-center>
            <c-button @click="handleConnectWallet">
              Connect Wallet
            </c-button>
          </c-center>
        </template>
      ...
      <c-modal-content>
        ...
        <c-modal-body>
          ...
          <template v-else-if="isMinting">
            Minting new NFT
            <c-animate-presence>
              <chakra.span backdrop-filter="blur(2px)" position="absolute" top="0" right="0" bottom="0" left="0" z-index="10" bg="blackAlpha.400" d="flex" justify-content="center" align-items="center">
