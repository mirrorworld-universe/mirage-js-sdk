    ...
    <template v-else>
      <c-grid v-if="nfts?.length" gap="4" :template-columns="['1fr', '1fr', 'repeat(4, auto)']">
        <RouterLink v-for="(nft, i) in nfts" :key="`nft-i-${i}`" custom :to="`/nft/${nft.mint}`" v-slot="{ navigate }">
          <chakra.div rounded="lg" overflow="hidden" backdrop-filter="blur(5px)" border-width="1px" border-style="solid" border-color="whiteAlpha.500">
...
async function fetchNfts(_address: string) {
  try {
    ...
    nfts.value = await Promise.all(
      (
        await mirage.value!.getUserNfts(new PublicKey(_address))
      ).map(async (token) => {
